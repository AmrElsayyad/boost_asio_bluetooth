...
class Acceptor : public boost::enable_shared_from_this<Acceptor> {
  ...
  private:
    ...
    virtual bool OnAccept(boost::shared_ptr<Connection> connection,
                          const std::string &addr, boost::uint8_t channel) = 0;
  ...
  public:
    ...
    // Begin listening on a specific bluetooth interface.
...
class Connection : public boost::enable_shared_from_this<Connection> {
  ...
  private:
    ...
    void HandleSend(const boost::system::error_code &error,
                    std::list<std::vector<boost::uint8_t> >::iterator itr);
    void HandleRecv(const boost::system::error_code &error,
                    boost::int32_t actual_bytes);
    void HandleTimer(const boost::system::error_code &error);
