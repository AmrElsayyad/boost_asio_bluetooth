...
boost::asio::io_service &Hive::GetService() { return m_io_service; }

// Hive::HasStopped definition
bool Hive::HasStopped() {
    return (boost::interprocess::ipcdetail::atomic_cas32(&m_shutdown, 1, 1) ==
            1);
}
...
// Hive::Run definition
void Hive::Run() { m_io_service.run(); }
    ...
    if (error || HasError() || m_hive->HasStopped()) {
        ...
        if (connection->GetSocket().is_open()) {
            ...
            if (OnAccept(connection,
                         ...
                         connection->GetSocket().remote_endpoint().channel())) {
                connection->OnAccept(m_acceptor.local_endpoint().address(),
                                     m_acceptor.local_endpoint().channel());
            }
        } else {
            std::cout << "Acceptor::HandleAccept has error!" << std::endl;
            StartError(error);
...
void Acceptor::Stop() {
    m_io_strand.post(boost::bind(&Acceptor::HandleTimer, shared_from_this(),
...
void Acceptor::Accept(boost::shared_ptr<Connection> connection) {
    m_io_strand.post(
        boost::bind(&Acceptor::DispatchAccept, shared_from_this(), connection));
}

// Acceptor::Listen definition with endpoint
inline void Acceptor::Listen(
    const boost::asio::bluetooth::bluetooth::endpoint &endpoint) {
...
// Acceptor::Listen on a specific channel definition
void Acceptor::Listen(uint8_t channel) {
    Listen(boost::asio::bluetooth::bluetooth::endpoint(channel));
}

// Acceptor::Listen on a specific bluetooth interface definition
...
boost::shared_ptr<Hive> Acceptor::GetHive() { return m_hive; }
...
int32_t Acceptor::GetTimerInterval() const { return m_timer_interval; }
...
bool Acceptor::HasError() {
    return (boost::interprocess::ipcdetail::atomic_cas32(&m_error_state, 1,
                                                         1) == 1);
}
...
void Connection::StartSend() {
    if (!m_pending_sends.empty()) {
        boost::asio::async_write(
            m_socket, boost::asio::buffer(m_pending_sends.front()),
            m_io_strand.wrap(boost::bind(
                &Connection::HandleSend, shared_from_this(),
...
void Connection::StartError(const boost::system::error_code &error) {
    if (boost::interprocess::ipcdetail::atomic_cas32(&m_error_state, 1, 0) ==
        ...
        boost::system::error_code ec;
        m_socket.shutdown(
            boost::asio::bluetooth::bluetooth::socket::shutdown_both, ec);
        m_socket.close(ec);
        m_timer.cancel(ec);
        OnError(error);
    }
...
void Connection::HandleConnect(const boost::system::error_code &error) {
    ...
    if (error || HasError() || m_hive->HasStopped()) {
        ...
        if (m_socket.is_open())
            OnConnect(m_socket.remote_endpoint().address(),
                      m_socket.remote_endpoint().channel());
        else {
            StartError(error);
        }
    }
...
void Connection::HandleRecv(const boost::system::error_code &error,
    ...
    if (error || HasError() || m_hive->HasStopped())
        ...
        if (!m_pending_recvs.empty()) StartRecv(m_pending_recvs.front());
    }
}
...
void Connection::DispatchSend(std::vector<uint8_t> buffer) {
    ...
    if (should_start_send) StartSend();
}
...
void Connection::DispatchRecv(int32_t total_bytes) {
    ...
    if (should_start_receive) StartRecv(total_bytes);
}
...
boost::asio::io_context::strand &Connection::GetStrand() { return m_io_strand; }
#else
...
boost::shared_ptr<Hive> Connection::GetHive() { return m_hive; }
...
int32_t Connection::GetTimerInterval() const { return m_timer_interval; }
