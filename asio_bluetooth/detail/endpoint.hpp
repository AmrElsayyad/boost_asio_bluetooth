...
class endpoint {
  public:
    ...
    endpoint(uint8_t channel) : data_() {
        data_.bt.rc_family = AF_BLUETOOTH;
        data_.bt.rc_bdaddr = (bdaddr_t){0};
    ...
    endpoint(const char* mac_addr, uint8_t channel) : data_() {
        data_.bt.rc_family = AF_BLUETOOTH;
        char addr[18];
    ...
    endpoint(const std::string& mac_addr, uint8_t channel) : data_() {
        data_.bt.rc_family = AF_BLUETOOTH;
        char addr[18];
    ...
    boost::asio::detail::socket_addr_type* data() { return &data_.base; }
    ...
    uint8_t channel() const { return (uint8_t)data_.bt.rc_channel; }
